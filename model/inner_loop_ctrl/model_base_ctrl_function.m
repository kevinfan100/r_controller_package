...
%     Uses modified ZPETC (only cancels minimum-phase zero ba)
%     3-state estimator (d=0) with extended control law
%
% Inputs:
...
% See also: model_base_ctrl_params, lpf_system_params, CLAUDE.md
    ...
    if params.lpf_enable > 0.5
        ...
        % ZPETC for LPF mode: only cancel minimum-phase zero ba
        % Non-minimum phase zero bu is NOT canceled (ZPETC limitation)
        % vf[k] = λf*vf[k-1] + kff_lpf{ba*vd[k] + (1-ba*λc)*vd[k-1] - λc*vd[k-2]}
        if params.ff_enable > 0.5
            vf_k = params.lambda_f * vf_k1 + params.kff_lpf * ...
                   (params.ba * vd + params.one_S_ba_M_lambda_c * vd_k1 - params.lambda_c * vd_k2);
        else
        ...
        % Note: This formula remains the same regardless of ff_enable
        delta_vf = vf_k - params.lambda_c_A_kc * vf_k1 - params.bc * vf_k2;

        % δv[k] = vf[k] - vm[k]
        delta_v = vf_k - vm;

        %% Estimator (Disturbance Observer) - Same as 2nd-order
        ...
        % 3rd-order control law:
        % uc[k] = (λc + kc)·uc[k-1] + bc·uc[k-2]
