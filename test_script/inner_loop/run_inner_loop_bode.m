...
% Create Bode plot tab with magnitude and phase subplots
    ...
    % Experiment data (markers only, no lines)
    h_ch = gobjects(6, 1);
    for ch = 1:6
        if ch == channel
        ...
        % Adjust marker size for square (P2) to match visual size of other shapes
        if ch == 2
            marker_size = ms * 1.15;  % Square appears smaller, scale up
        else
            marker_size = ms;
        end
    ...
    % Adjust marker size for square (P2) to match visual size of other shapes
    if channel == 2
        phase_marker_size = ms * 1.15;
    else
        phase_marker_size = ms;
    end
    ...
    ylim(ax2, [-90, 0]);  % Fixed phase range for comparison
    ax2.YTick = [-90, -60, -30, 0];
    ax2.XScale = 'log';
...
% Create preview comparison tab (log or linear frequency scale)
    ...
    for preview_idx = 1:num_fft_previews
        ...
        % Adjust marker size for square ('s') to match visual size of other shapes
        if strcmp(preview_markers{color_idx}, 's')
            marker_size = ms * 1.15;
        else
            marker_size = ms;
        end
