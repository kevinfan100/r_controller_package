...
particle_params_test = particle_dynamics_params('HInit', 5);  % default wall (w_hat = [0;0;1])
p0 = [0; 0; 5];

% Test at t=0
p_d_0 = trajectory_generator_function(0, p0, traj_params_sine.Value, particle_params_test.Value);
...
p_d_025 = trajectory_generator_function(0.25, p0, traj_params_sine.Value, particle_params_test.Value);
...
p_d_end = trajectory_generator_function(5, p0, traj_params_sine.Value, particle_params_test.Value);
...
p_d_circle_0 = trajectory_generator_function(0, p0, traj_params_circle.Value, particle_params_test.Value);
...
% Test positioning
traj_params_pos = trajectory_generator_params('Type', 2, 'HoldTime', 5);
p_d_pos = trajectory_generator_function(2.5, p0, traj_params_pos.Value, particle_params_test.Value);
fprintf('positioning at t=2.5: [%.2f, %.2f, %.2f] (expected: [0, 0, 5])\n', p_d_pos(1), p_d_pos(2), p_d_pos(3));
assert(all(abs(p_d_pos - p0) < 1e-10), 'Positioning should always return p0');
...
for k = 1:N_steps
    ...
    p_d = trajectory_generator_function(t, p0, traj_params.Value, particle_params.Value);
